...
TEST (rpc, wallet_list)
{
    ...
    rai::rpc rpc (system.service, *system.nodes[0], rai::rpc_config (true));
    rpc.start ();
    std::vector<rai::uint256_union> input;
    rai::keypair key2;
    rai::keypair key3;
    input.push_back(key3.pub);
    input.push_back(key2.pub);
    input.push_back(system.nodes[0]->wallets.items.begin ()->first); // wallets.items[0] generated by system()
    (void) system.nodes[0]->wallets.create (key2.pub); // return values aren't needed, cast to (void)
    (void) system.nodes[0]->wallets.create (key3.pub);
    boost::property_tree::ptree request;
    ...
    ASSERT_EQ (200, response.status);
    auto & node_wallets (response.json.get_child ("wallets"));
    std::vector<rai::uint256_union> output;
    for (auto i : node_wallets)
    {
        auto wallet_id (i.second.get<std::string> (""));
        output.push_back(wallet_id);
    }
    ASSERT_EQ (input.size(), output.size ());
    std::vector<rai::uint256_union>::iterator i;
    std::vector<rai::uint256_union>::iterator j;
    for ( i = input.begin(), j = output.begin(); i != input.end() && j != output.end(); ++i, ++j)
    {
        ASSERT_EQ (i->number(), j->number());
    }
}
